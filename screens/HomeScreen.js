...
import { InputAddRemont } from "../src/components/InputAddRemont";
...
const MainScreen = ({ navigation }, setValue) => {
  ...
  const listener = () => {
    ...
    // ScreenOrientation.getOrientationAsync().then((letqScreenOrientation) => console.log(letqScreenOrientation));
    if (
      ScreenOrientation.getOrientationAsync() == 1 ||
      ScreenOrientation.getOrientationAsync() == 2
    ) {
      setOrientation(12);
    } else {
      setOrientation(34);
  ...
  myHeaders.append("Authorization", KEYS_RIC.AUTHORIZATION_KEY);
  // Опции запроса POST
  ...
  const [scooterGoBrokenDisabledButton, setscooterGoBrokenDisabledButton] =
    useState(false);
  const [
    scooterGoAvailableDisabledButton,
    setscooterGoAvailableDisabledButton,
  ] = useState(false);
  const [
    scooterGoOpenBatteryDisabledButton,
    setscooterGoOpenBatteryDisabledButton,
  ] = useState(false);
  ...
  const objectsInSuper = useSelector(
  ...
  const scooterGoCommand = async (GoCommand) => {
    ...
    if (objectsInSuper.length == 0) {
      ...
      const dataResponseFireBase = await responseFireBase.json();
      const dataView = Object.keys(dataResponseFireBase).map((key) => ({
        ...dataResponseFireBase[key],
        id: key,
      }));
      let API_RIC_KEY = dataView[0].title;

      // Получаем список объектов от RIC
      const RIC_OBJECTS_LIST = await fetch(
      ...
      try {
        ...
        Alert.alert(
          "Вы не предоставили разрешение на использование гео-позиции (перейдите в настройки)"
        );
      }
      ...
      // Если ошибки нет, запускаем:
        ...
        for (var i = 0; i < objectsInSuper.length; i++) {
          ...
          for (let j = 0; j < data_RIC_OBJECTS_LIST.length; j++) {
            // Ищем отсканированный объект в списке объектов RIC
            if (
              data_RIC_OBJECTS_LIST[j].config.data.qr == objectsInSuper[i].title
            ) {
              IDsForLabels.push(`${data_RIC_OBJECTS_LIST[j]._id}`);
              ...
              try {
                ...
                const api_url_scooterlockall = await fetch(url_go_command, {
                  method: "POST",
                  headers: {
                    Authorization: API_RIC_KEY,
                  },
                ...
                if (statusResponse == 422) {
                  ...
                  if (titleResponse == "error_api_already_broken") {
                    ...
                    titleResponse ==
                    "error_api_cant_change_from_taken_to_broken"
                  ) {
                    console.log("  Ответ: Самокат в аренде");
                    addTodo22(
                      numQrScooter,
                      objectStatusOnline,
                      "Ошибка (Самокат в аренде)"
                    );
                    eroorExistsGoBroken = eroorExistsGoBroken + 1;
                  } else if (
                    titleResponse ==
        ...
        if (GoCommand == "GoBroken") {
          ...
          await fetch(
            "https://app.rightech.io/api/v1/links/labels/to/objects",
            {
              method: "POST",
              headers: {
                Authorization:
            ...
            .then((response) =>
              console.log("Метка склад (статус): " + response.status)
            )
            .catch((error) => console.log("error", error));
          ...
          await fetch(
            ...
            .then((response) =>
              console.log("Метка город анлинк (статус): " + response.status)
            )
            .catch((error) => console.log("error", error));
        } else if (GoCommand == "GoAvailable") {
          console.log("ПЕРЕВОЖУ МЕТКИ В ГОРОД");
          ...
          await fetch(
            ...
            .then((response) =>
              console.log("Метка город (статус): " + response.status)
            )
            .catch((error) => console.log("error", error));
          ...
          await fetch(
            ...
            .then((response) =>
              console.log("Метка склад анлинк (статус): " + response.status)
            )
            .catch((error) => console.log("error", error));
        }
        ...
        const data = await response.json();
        // console.log('DATATA: ', data)
        const dataView = Object.keys(data).map((key) => ({
          ...data[key],
          id: key,
        }));
        let API_TELEGRAM_KEY = dataView[1].title;
        ...
        // console.log('КЛЮЧ: ' + API_TELEGRAM_KEY)
        // Асинхронная функция на axios для отправки POST запроса, для отправки сообщения в Телеграм
        const TG_MESSAGE_PUSH = await axios.post(
        ...
        if (TG_MESSAGE_PUSH_StatusResponse == 200) {
          ...
          addTodo22(
            "Telegram",
            TG_MESSAGE_PUSH_StatusResponse,
            "Не отправлено!"
          );
        }
        ...
        // console.log(api_urlTG.status)
        // Асинхронная функция на axios для отправки POST запроса, для отправки гео-позиции устройства в Телеграм
        const TG_LOCATION_PUSH = await axios.post(
          KEYS_TELEGRAM.URI_API_LOCATION,
          {
            chat_id: TELEGRAM_KEY_CHAT_ID,
            latitude: x,
            longitude: y,
  ...
  const scooterGoAvailable = async () => {
    ...
    // Если ошибки нет, запускаем:
      ...
      for (var i = 0; i < todos.length; i++) {
        ...
        for (let j = 0; j < dataObjectsListCount.length; j++) {
          ...
          if (dataObjectsListCount[j].config.data.qr == todos[i].title) {
            ...
            try {
              const api_url_scooterlockall = await fetch(
                `https://app.rightech.io/api/v1/objects/${dataObjectsListCount[j]._id}/commands/change-status-available?withChildGroups=true`,
                {
                  method: "POST",
                  headers: {
                    Authorization: API_RIC_KEY,
      ...
      const scooterListSendToTelegramAvailable = async () => {
        ...
        // console.log('КЛЮЧ: ' + API_TELEGRAM_KEY)
        // Асинхронная функция на axios для отправки POST запроса, для отправки сообщения в Телеграм
        const api_urlTG = await axios.post(
          `https://api.telegram.org/bot${API_TELEGRAM_KEY}/sendMessage`,
          {
            chat_id: TELEGRAM_KEY_CHAT_ID,
            text: message,
        ...
        // console.log(api_urlTG.status)
        // Асинхронная функция на axios для отправки POST запроса, для отправки гео-позиции устройства в Телеграм
        const api_urlTG2 = await axios.post(KEYS_TELEGRAM.URI_API_LOCATION, {
          chat_id: TELEGRAM_KEY_CHAT_ID,
          latitude: x,
          longitude: y,
        });
      ...
      for (var i = 0; i < todos.length; i++) {
        ...
        for (let j = 0; j < dataObjectsListCount.length; j++) {
          ...
          if (dataObjectsListCount[j].config.data.qr == todos[i].title) {
            ...
            try {
              const api_url_scooterlockall = await fetch(reqs, {
                method: "POST",
                headers: {
                  Authorization: API_RIC_KEY,
                },
  ...
  const pressDelete = () => {
    if (todos.length == 0) {
      ...
      Alert.alert("Отчистить", "Вы уверены, что хотите отчистить список?", [
        {
          text: "Закрыть",
          onPress: () => console.log("Cancel Pressed"),
          style: "cancel",
  ...
  const inputAddNumberOpenValue = useSelector(
    (state) => state.post.inputAddNumberOpen
  );
  const inputAddNumberOpenValueRemont = useSelector(
    (state) => state.post.inputAddNumberOpenRemont
  );
  ...
  const allAddedObjectsArray = useSelector(
    (state) => state.post.allAddedObjectsArray
  );
  const resultsCommandsScootersArray = useSelector(
